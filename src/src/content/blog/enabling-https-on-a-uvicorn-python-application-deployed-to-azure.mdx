...
> **Update (February 2026):** The `ProxyHeadersMiddleware` approach below still works. If you're using Django, you can alternatively use the built-in [`SECURE_PROXY_SSL_HEADER`](https://learn.microsoft.com/en-us/azure/app-service/configure-language-python?WT.mc_id=AI-MVP-5004204#detect-https-session) setting (`SECURE_PROXY_SSL_HEADER = ('HTTP_X_FORWARDED_PROTO', 'https')`) which handles this natively without extra middleware.
