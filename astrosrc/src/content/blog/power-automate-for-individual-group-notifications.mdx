...
![](__GHOST_URL__/content/images/2021/03/image-18.png)An example of a very basic Adaptive Card, just saying hi!How do we do it?
----------------
...
![](__GHOST_URL__/content/images/2021/03/image-19.png)This scheduled cloud flow option lets you select a recurring frequencyComplete the form and select how often you'd like the flow to run.
...
![](__GHOST_URL__/content/images/2021/03/image-20.png)There are [tons of samples](https://adaptivecards.io/samples/Restaurant.html) to choose from on the adaptive cards site - and you can click 'Try it yourself' to edit a card in a WYSIWYG editor.
...
![](__GHOST_URL__/content/images/2021/03/image-21.png)An email from the automationAssuming the title is pretty explanatory, I'll create three compose actions, and add in my adaptive card, my email, and my title.

![](__GHOST_URL__/content/images/2021/03/image-22.png)You can see the recurrence trigger, the adaptive card, the email content, and the title.Looping over the users
----------------------
...
![](__GHOST_URL__/content/images/2021/03/image-23.png)I then create an 'Apply to each' action, and add the 'value' of the 'List group members' action to the selected output.

![](__GHOST_URL__/content/images/2021/03/image-24.png)Sending the messages - and the fallback
---------------------------------------
...
![](__GHOST_URL__/content/images/2021/03/image-30.png)Once this is created, I insert the Mail variable, for the users email - and I then insert the data I pre-prepared, with the advanced options expanded.
...
![](__GHOST_URL__/content/images/2021/03/image-25.png)### And... the email fallback
...
![](__GHOST_URL__/content/images/2021/03/image-26.png)Once that's complete, enter 403 in the right hand side - so your condition should be checking whether the status code from posting via the Flow bot, is equal to 403.

![](__GHOST_URL__/content/images/2021/03/image-27.png)Hit the ellipses '...' in the condition, and select 'Configure run after.'

![](__GHOST_URL__/content/images/2021/03/image-28.png)Ensure the condition runs if the bot is successful, fails, or is skipped.

![](__GHOST_URL__/content/images/2021/03/image-29.png)Now, if the bot fails to send a message - because the user has blocked it - we should catch that error.
...
![](__GHOST_URL__/content/images/2021/03/image-31.png)Complete the connector, entering the 'Mail' output in the 'To' box, and the 'Title' output in the 'Subject' box.

Once complete, select the code icon, on the furthest right hand side of the WYSIWYG editor, to enter HTML entry mode.

![](__GHOST_URL__/content/images/2021/03/image-33.png)Once you're in the HTML editor mode, hit 'Add dynamic content', and then select the HTML email we created earlier.

![](__GHOST_URL__/content/images/2021/03/image-36.png)![](__GHOST_URL__/content/images/2021/03/image-32.png)Time to run!
------------
...
![](__GHOST_URL__/content/images/2021/03/image-38.png)For users that have blocked the bot, you'll see the condition evaluate as true, and the fallback email will have been sent.

![](__GHOST_URL__/content/images/2021/03/image-37.png)Thanks for reading - and I hope you find this useful. Remember not to spam your users!
